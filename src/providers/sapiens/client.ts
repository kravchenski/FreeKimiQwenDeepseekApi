...
export async function sapiensCompletion(options: CompletionOptions) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Sapiens (ZenMux) completion failed: ${response.status} ${text}`);
  }

  return { response };
}

export function parseSapiensEvent(raw: string, state?: Record<string, any>): Record<string, any> | null {
