...
export async function stepfunCompletion(options: CompletionOptions) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`StepFun (ZenMux) completion failed: ${response.status} ${text}`);
  }

  return { response };
}

export function parseStepfunEvent(raw: string, state?: Record<string, any>): Record<string, any> | null {
